...
export default {
  feriadosAPI(url = URL_BASE) {
    ...
    return ({
      getAll: () => axios.get(URL_BASE + URL_GetAll),
      getOne: (id) => axios.get(URL_BASE + URL_GetOne + id),
      update: (id, rec) => axios.put(URL_BASE + URL_UpdOne + id),
      preload: (recs) => axios.post(URL_BASE + URL_PreLd, recs),
      // delete: (id) => axios.delete(URL_BASE + URL_GetOne + id), // not used
